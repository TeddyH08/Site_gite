...
/* ReCaptch Secret */
$recaptchaSecret = '6LeTUL4UAAAAAGeuKKP7Qz8SIKCG0M7japsJcZCW';

$dzEmailTo 		= "contact@gite-beauregard.fr";   /* Receiver Email Address */
$dzEmailFrom    = "Site Web Gite Beauregard";
...
try {
    if (!empty($_POST)) {
		...
		{
			$dzRes['status'] = 0;
			$dzRes['msg'] = 'Probleme de format mail.';
		}
		$dzMailSubject = 'Site Beauregard, un contact vous envoie un message';
		$dzMailMessage	= 	"
							Une personne vous contacte: <br><br>
							Name: $dzName<br/>
							Email: $dzEmail<br/>
		...
		{
			$dzRes['status'] = 1;
			$dzRes['msg'] = 'Nous avons reçu votre message, à bientôt.';
		}
		else
		{
			$dzRes['status'] = 0;
			$dzRes['msg'] = 'Erreur, merci de refaire votre message.';
		}
		echo json_encode($dzRes);
		exit;
    ...
    $dzRes['status'] = 0;
	$dzRes['msg'] = $e->getMessage().'Erreur, merci de refaire votre message.';
	echo json_encode($dzRes);
	exit;
}
